...
Post frontmatter must satisfy the content schema: `title`, `date`, `description`, optional `tag`, and optional `image`, and optional `draft` (defaults to `false`). Set `draft: true` to keep a post accessible by direct URL while excluding it from public listings, RSS, sitemap, and ActivityPub publication. When editing deployment-related code, review `astro.config.ts`, `wrangler.jsonc`, and `setup.sql` together so Astro output, Cloudflare bindings, and database expectations stay aligned.
