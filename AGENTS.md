...
`src/` contains the Astro app. Page routes live in `src/pages/`, shared layouts in `src/layouts/`, reusable UI in `src/components/`, content helpers in `src/utils/`, and site data in `src/data/` plus `src/config.ts`. Blog posts are stored as Markdown or MDX in `src/content/posts/` and validated by `src/content.config.ts`. Static files go in `public/`. The Cloudflare Worker entrypoint is `functions/index.ts`; its ActivityPub handlers and helpers live under `functions/`.
...
- `pnpm worker:dev`: build and preview the site with the local Workers runtime.
- `pnpm worker:check`: type-check the Worker.
- `pnpm test:worker`: bundle and run the Worker regression tests (run `pnpm build` first).
