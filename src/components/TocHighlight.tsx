...
const TocHighlight: Component<{ headings: TocHeading[] }> = (props) => {
  ...
  const [activeIndexes, setActiveIndexes] = createSignal<Set<number>>(
    new Set(),
  );
  ...
  const update = () => {
    ...
    setPositions(
      currentItems.map((item) => getHeadingScrollPosition(item.heading) * 100),
    );
    ...
    const fallbackIndex = currentItems.reduce((current, item, index) => {
      ...
      if (isVisible) visibleIndexes.add(index);

      return rect.top <= 112 ? index : current;
    }, 0);

    setActiveIndexes(
      visibleIndexes.size > 0 ? visibleIndexes : new Set([fallbackIndex]),
    );
  };
  ...
  onMount(() => {
    ...
    const lineItems = props.headings
      ...
      .map((heading) => {
        const target = articleHeadings.find(
          (articleHeading) => articleHeading.id === heading.slug,
        );
    ...
    >
      <For each={items()}>
        {(item, index) => {
          ...
          const positionStyle = () =>
            `--toc-position: ${positions()[index()] ?? 50}%`;
